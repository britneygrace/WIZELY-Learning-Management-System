...
<body>
	...
	<form action="" method="POST">
		...
		if (isset($_POST['submitanswer'])) {
			...
			$query = "SELECT * FROM quizrecords WHERE Name='$login_user' AND Category='CSS Quiz'";
			$result = $conn->query($query);
			// while ($row = $res->fetch(PDO::FETCH_ASSOC)) {
			if ($result->rowcount() > 0) {
				echo '<h4 style="text-align:right; margin-right:80px">YOU ALREADY TOOK THE QUIZ</h4>';
			} else {
				$sql1 = "INSERT INTO quizrecords (userId, Category, Name, Score) VALUES ('$user_id', 'CSS Quiz', '$login_user', '$res')";
